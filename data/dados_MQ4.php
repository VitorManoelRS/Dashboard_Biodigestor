...
$query = "SELECT time, Metano_ppm FROM sensor ORDER BY time DESC "; // altere para a sua tabela e colunas
...
while ($row = $result->fetch_assoc()) {
    $labels[] = date('d-m H:i:s', strtotime($row['time'])); // formata a data/hora para exibir no gráfico
    $data[] = floatval($row['Metano_ppm']); // converte o valor para float e armazena no array
}
...
echo json_encode([
  'labels' => array_slice($labels,$qnt_valores), 
  'data' => array_slice($data,$qnt_valores) 
]);
